...
import { getProjects, boostProject } from "@/lib/card-manager"
...
export default function Home() {
  ...
  const handleSwipeRight = async () => {
    ...
    // 2. Balance Check
    // We add a small buffer (0.01) to cover the gas fee since we are paying gas with the stablecoin (Fee Abstraction)
    const gasBuffer = parseEther("0.01")
    if (tokenBalance === undefined || tokenBalance < amountWei + gasBuffer) {
      alert(`Insufficient balance. You need ${amountNum} ${donationCurrency} plus a small amount for gas (paid in ${donationCurrency}).`)
      return
    }
    ...
    try {
      ...
      if (allowance === undefined || allowance < amountWei) {
        ...
        const approveHash = await writeContractAsync({
          ...
          feeCurrency: tokenAddress, // Pay gas with the stablecoin
        })
