...
export function ShareModal({ project, isOpen, onClose }: ShareModalProps) {
  ...
  const shareUrl = `https://swipepad.xyz/project/${project.id}`
    ...
    try {
      ...
      switch (platform) {
        ...
        case "telegram":
          ...
          break
        case "farcaster":
          shareLink = `https://warpcast.com/~/compose?text=${encodeURIComponent(shareText)}&embeds[]=${encodeURIComponent(shareUrl)}`
          break
        default:
          if (navigator.share) {
            navigator
      ...
      <div className="bg-[#1F2732] rounded-xl w-full max-w-md p-6 shadow-xl">
        ...
        <div className="space-y-4">
          <div>
            ...
            >
              <MessageSquare className="w-4 h-4 mr-2" /> Share with your contacts
            </button>
          </div>

          <div>
            <h4 className="text-sm font-medium mb-3">Share project in social media</h4>
            <div className="grid grid-cols-4 gap-3">
              <button
                onClick={() => handleExternalShare("farcaster")}
          ...
          <div>
            ...
            <div className="flex items-center">
              ...
              <button
                ...
                className={`py-2 px-3 rounded-r-lg transition-colors ${copied ? "bg-green-600 text-white" : "bg-gray-700 hover:bg-gray-600 text-white"
                  }`}
              >
