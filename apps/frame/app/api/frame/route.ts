...
async function handleDonate(projectIndex: number, category: string, userAddress: string) {
  ...
  const imageUrl = `${process.env.NEXT_PUBLIC_FRAME_BASE_URL}/api/donate-image?projectId=${project.id}&category=${category}`;
...
async function handleBoost(projectIndex: number, category: string) {
  ...
  const imageUrl = `${process.env.NEXT_PUBLIC_FRAME_BASE_URL}/api/boost-image?projectId=${project.id}&category=${category}`;
...
async function showProject(projectIndex: number, category: string) {
  ...
  const { data: projects, error } = await supabase.rpc('get_shuffled_deck', {
    p_category: category,
    p_user_address: '0x0000000000000000000000000000000000000000' // Use placeholder for now
  });
  ...
  const imageUrl = `${process.env.NEXT_PUBLIC_FRAME_BASE_URL}/api/image?projectId=${project.id}&category=${category}`;
